...
public class PrimaryController {
    ...
    public void turno(){
        ...
        // jogador pede/recebe carta se ainda não parou
        if (!jogo.jogador.isParou()) {
            Carta c = jogo.distribuirCarta(jogo.jogador);
            if (c != null) {
                mesaDoJogador.getChildren().add(imagemCarta(c)); 
            }
        }

        // computador age segundo sua estratégia
        if (!jogo.computador.deveParar()) {
            Carta c2 = jogo.distribuirCarta(jogo.computador);
            if (c2 != null) {
                mesaDoComputador.getChildren().add(imagemCarta(c2)); 
            }
        }

        // se computador já atingiu sua condição de parada, marca como parado
        if (jogo.computador.deveParar()) {
